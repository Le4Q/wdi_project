...
public class RestaurantBlockingKeyByCategoryGenerator extends RecordBlockingKeyGenerator<Restaurant, Attribute> {
    ...
    public void generateBlockingKeys(Restaurant record, Processable<Correspondence<Attribute, Matchable>> correspondences,
        ...
        if (categories.size() != 0) {
            String smallestCategory = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";

            for (String category: categories) {
                if (category.length() < smallestCategory.length() && category.length() > 2) {
                    smallestCategory = category;
                }
            }
            resultCollector.next(new Pair<>(smallestCategory, record));
        }
    }
}
