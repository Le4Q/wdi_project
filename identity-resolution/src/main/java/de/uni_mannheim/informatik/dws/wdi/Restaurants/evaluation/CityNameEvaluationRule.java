...
public class CityNameEvaluationRule extends EvaluationRule<Restaurant, Attribute> {
    ...
    public boolean isEqual(Restaurant record1, Restaurant record2, Attribute attribute) {
        if(record1.getPostalAddress().getCity().getName() == null && record2.getPostalAddress().getCity().getName() == null)
            return true;
        else if(record1.getPostalAddress().getCity().getName() == null ^ record2.getPostalAddress().getCity().getName() == null)
            return false;
        else
            return sim.calculate(record1.getPostalAddress().getCity().getName(), record2.getPostalAddress().getCity().getName()) > 0.7;
    }
