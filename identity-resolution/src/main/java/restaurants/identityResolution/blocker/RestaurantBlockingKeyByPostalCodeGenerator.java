 ...
 * 
 */
public class RestaurantBlockingKeyByPostalCodeGenerator extends
	...
	public void generateBlockingKeys(Restaurant record, Processable<Correspondence<Attribute, Matchable>> correspondences,
		...
		String postalcode = record.getPostalAddress().getCity().getPostalCode();

		if  (postalcode != null) {

			blockingKeyValue += postalcode.substring(0, Math.min(3, postalcode.length())).toUpperCase();
